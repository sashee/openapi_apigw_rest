...
resource "aws_s3_bucket_object" "api_object" {
  key    = "api.yml"
  content = templatefile("api.yml", {api_url = aws_api_gateway_deployment.deployment.invoke_url, users_lambda_arn = "", user_lambda_arn = ""})
  bucket = aws_s3_bucket.bucket.bucket
}
...
resource "aws_lambda_permission" "apigw" {
  ...
  function_name = local.lambdas[count.index].function_name
